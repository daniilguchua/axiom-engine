...
def test_diagnostics_table():
    ...
    if (first['mode'] != test_data['mode'] or 
        first['difficulty'] != test_data['difficulty'] or
        first['llm_step_count'] != test_data['llm_step_count']):
...
def test_logging_helper():
    ...
    try:
        ...
        expected_params = [
            'cache_manager', 'session_id', 'mode', 'difficulty',
            'llm_raw', 'new_steps', 'cleaned_steps', 'storage_before',
            'storage_after', 'integrity_pass', 'integrity_error'
        ]
        
        if params == expected_params:
            print(f"   ✅ Correct signature ({len(params)} parameters)")
        else:
            print(f"   ⚠️ Signature mismatch")
