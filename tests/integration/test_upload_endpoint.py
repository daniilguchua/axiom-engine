...
class TestUploadEndpointBasic:
    ...
    def test_upload_requires_file(self, flask_client, monkeypatch):
        ...
        with patch("core.config.get_configured_api_key", return_value="test-key"):
            response = flask_client.post("/upload", data={}, headers={"X-Session-ID": "test-session-123"})
    ...
    def test_upload_rejects_wrong_content_type(self, flask_client, monkeypatch):
        ...
        with patch("core.config.get_configured_api_key", return_value="test-key"):
            response = flask_client.post(
                "/upload", json={"text": "not a file"}, headers={"X-Session-ID": "test-session-123"}
            )
...
class TestUploadWorkflow:
    ...
    def test_full_upload_workflow(
        self, mock_create_store, mock_extract, mock_session_mgr, mock_api_key, flask_client, monkeypatch
    ):
        ...
        # Mock PDF extraction
        mock_extract.return_value = (["Sample content"], [{"source": "test.pdf", "page": 1}], 1)
        ...
        # Create test file
        file_data = {"file": (io.BytesIO(b"%PDF-1.4 fake content"), "test.pdf")}

        response = flask_client.post(
            "/upload", data=file_data, headers={"X-Session-ID": "test-session-123"}, content_type="multipart/form-data"
        )
...
class TestUploadErrorHandling:
    ...
    def test_upload_handles_missing_api_key(self, flask_client, monkeypatch):
        ...
        with patch("core.config.get_configured_api_key", return_value=None):
            file_data = {"file": (io.BytesIO(b"content"), "test.pdf")}
    ...
    def test_upload_handles_invalid_file_format(self, flask_client, monkeypatch):
        ...
        with patch("core.config.get_configured_api_key", return_value="test-key"):
            ...
            file_data = {"file": (io.BytesIO(b"not pdf"), "document.txt")}
...
class TestUploadSessionUpdates:
    ...
    def test_upload_stores_filename_in_session(self):
        ...
        session = {"filename": None, "vector_store": None}
