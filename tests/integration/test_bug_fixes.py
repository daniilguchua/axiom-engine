...
response = client.post("/chat", json={"message": "test"}, headers={"X-Session-ID": "invalid@#$"})
print(f"   Status: {response.status_code} - {'✅ PASS' if response.status_code == 401 else '❌ FAIL'}")
...
response = client.post("/upload", json={"text": "not a file"}, headers={"X-Session-ID": "test-session-123"})
