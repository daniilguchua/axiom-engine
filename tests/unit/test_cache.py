...
class TestCacheManagerSetup:
    ...
    def test_database_initialization(self, temp_db_path, monkeypatch):
        ...
        monkeypatch.setenv("GEMINI_API_KEY", "test-key")
        CacheManager(db_path=temp_db_path)
...
class TestSemanticSearch:
        ...
        with manager._get_connection() as conn:
            ...
            cursor.execute(
                """
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """,
                ("cached-prompt", json.dumps(db_embedding), json.dumps(playlist_data), "engineer", 1),
            )

        # Query should find it via semantic similarity
        with patch("core.cache.semantic_cache.cosine_similarity", return_value=0.95):
            result = manager.get_cached_simulation("similar prompt", difficulty="engineer")
    ...
    def test_cache_miss_with_low_similarity(self, mock_embed, temp_db_path, monkeypatch):
        ...
        with patch("core.cache.semantic_cache.cosine_similarity", return_value=0.5):
            result = manager.get_cached_simulation("different prompt", difficulty="engineer")
            ...
            for difficulty in ["explorer", "engineer", "architect"]:
                cursor.execute(
                    """
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """,
                    (f"prompt-{difficulty}", json.dumps([1.0, 0.0, 0.0]), json.dumps({"steps": []}), difficulty, 1),
                )

        # Query with specific difficulty
        with patch("core.cache.semantic_cache.cosine_similarity", return_value=0.95):
            result = manager.get_cached_simulation("test prompt", difficulty="explorer")
    ...
    def test_cache_requires_complete_status(self, mock_embed, temp_db_path, monkeypatch):
        ...
        with patch("core.cache.semantic_cache.cosine_similarity", return_value=0.95):
            result = manager.get_cached_simulation("test prompt", difficulty="engineer")
        ...
        with manager._get_connection() as conn:
            ...
            cursor.execute(
                """
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                (prompt_hash, json.dumps({"steps": [{"code": "graph LR; X-->Y"}]}), "engineer", 0),
            )
...
class TestAccessMetrics:
    ...
    def test_save_and_retrieve_simulation(self, mock_embed, temp_db_path, monkeypatch):
        ...
        # Retrieve it
        result = manager.get_cached_simulation(prompt="test prompt for access", difficulty="engineer")
...
class TestBrokenSimulationTracking:
    ...
    def test_mark_simulation_broken(self, temp_db_path, monkeypatch):
        ...
        result = manager.mark_simulation_broken(
            prompt="test-prompt", difficulty="engineer", reason="Syntax error in node declaration"
        )
...
class TestPendingRepairs:
    ...
    def test_mark_repair_pending(self, temp_db_path, monkeypatch):
        ...
        manager.mark_repair_pending(session_id="session-1", prompt_key="test-prompt", step_index=1)
    ...
    def test_mark_repair_resolved(self, temp_db_path, monkeypatch):
        ...
        # Then resolve it
        manager.mark_repair_resolved(session_id="session-1", prompt_key="test-prompt", step_index=1)
...
class TestCacheThreadSafety:
    ...
    def test_concurrent_cache_saves(self, mock_embed, temp_db_path, monkeypatch):
        ...
        def save_sim(index):
            try:
                manager.save_simulation(
                    prompt=f"prompt-{index}", playlist_data={"steps": []}, difficulty="engineer", is_final_complete=True
                )
