    ...
    function formatInputDataBadge(inputData) {
        ...
        switch(type) {
            case 'array': {
                ...
                return `📥 Input Array \u00B7 ${n} elements`;
            }
            case 'tree': {
                const n = Array.isArray(value) ? value.length : 0;
                return `🌳 BST Input \u00B7 ${n} nodes`;
            }
            case 'linkedlist': {
                const n = Array.isArray(value) ? value.length : 0;
                return `🔗 Linked List \u00B7 ${n} nodes`;
            }
            case 'graph': {
                const n = value && typeof value === 'object' ? Object.keys(value).length : 0;
                return `🗺️ Graph \u00B7 ${n} nodes`;
            }
            case 'search': {
            ...
            case 'dp':
                return `⚡ DP Instance`;
            case 'hashtable': {
                const size = (value && value.table_size) || 7;
                return `#️⃣ Hash Table \u00B7 size ${size}`;
            }
            default:
                return `📥 Input Data`;
        }
