...
(function() {
    ...
    function togglePresets(show) {
        if (show) {
            ...
            if (presetSearch) presetSearch.focus();
        } else {
    ...
    function applyPresetFilter(query) {
        const searchTerm = query.trim().toLowerCase();
        const categories = document.querySelectorAll('.preset-category');

        categories.forEach(category => {
            const subject = category.dataset.subject || 'cs';
            const subjectMatches = activePresetSubject === 'all' || subject === activePresetSubject;
            ...
            const buttons = category.querySelectorAll('.preset-btn');
            let anyVisible = false;

            buttons.forEach(button => {
                const label = button.textContent.toLowerCase();
                const prompt = (button.getAttribute('data-prompt') || '').toLowerCase();
                const matches = !searchTerm || label.includes(searchTerm) || prompt.includes(searchTerm);
    ...
    subjectButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            activePresetSubject = button.dataset.subject || 'all';
            applyPresetFilter(presetSearch ? presetSearch.value : '');
        });
    });
    ...
    if (cardContext && fileInput) {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            try {
                ...
                // Show document badge in input area
                const docBadge = document.getElementById('doc-badge');
                const docBadgeName = document.getElementById('doc-badge-name');
                if (docBadge && docBadgeName) {
                    docBadgeName.textContent = data.filename || file.name;
                    docBadge.style.display = 'flex';
                    AXIOM.state.documentLoaded = true;
                    AXIOM.state.documentName = data.filename || file.name;
                }
                
                AXIOM.ui.activateChatMode(`**SYSTEM INGEST COMPLETE**\nTarget Data: _${data.filename}_\n\n_${data.chunks} chunks indexed from ${data.pages} pages. You can now ask questions about this document or say "simulate the algorithm from the document" to visualize its content._`);
            } catch (e) {
    ...
    // Document badge close handler — event delegation for resilience
    document.querySelector('.input-wrapper')?.addEventListener('click', async (e) => {
        if (!e.target.closest('.doc-badge-close')) return;
        e.preventDefault();
        e.stopPropagation();
        const docBadge = document.getElementById('doc-badge');
        if (docBadge) docBadge.style.display = 'none';
        AXIOM.state.documentLoaded = false;
        AXIOM.state.documentName = null;
        try {
            await AXIOM.api.resetSession();
        } catch (err) {
            console.warn('[AXIOM:MAIN] Session reset failed:', err);
        }
        AXIOM.ui.showToast('Document removed');
    });
