...
from .database import DB_PATH, CacheDatabase
...
class CacheManager:
    ...
    def get_cached_simulation(self, prompt: str, difficulty: str = "engineer") -> dict[str, Any] | None:
        """
        ...
        """
        return self.semantic_cache.get_cached_simulation(prompt=prompt, difficulty=difficulty)
    ...
    def mark_simulation_broken(self, prompt: str, difficulty: str, reason: str = "") -> bool:
    ...
    def mark_repair_pending(self, session_id: str, prompt_key: str, step_index: int) -> None:
        """Mark that a step is being repaired."""
        self.repair_tracker.mark_repair_pending(session_id=session_id, prompt_key=prompt_key, step_index=step_index)

    def mark_repair_resolved(self, session_id: str, prompt_key: str, step_index: int, success: bool = True) -> None:
        """Mark a specific repair as resolved."""
        self.repair_tracker.mark_repair_resolved(
            session_id=session_id, prompt_key=prompt_key, step_index=step_index, success=success
        )

    def clear_pending_repairs(self, session_id: str, prompt_key: str) -> int:
        """Clear ALL pending repairs for a session/prompt."""
        return self.repair_tracker.clear_pending_repairs(session_id=session_id, prompt_key=prompt_key)
    ...
    def log_graph_sample(
        ...
        self.feedback_logger.log_graph_sample(
            code=code, context=context, source=source, was_repaired=was_repaired, quality_score=quality_score
        )
    ...
    def export_training_data(self, output_path: str) -> int:
        ...
        with self.database.get_connection() as conn:
            ...
            data = [
                {"code": row[0], "context": row[1], "source": row[2], "was_repaired": bool(row[3]), "quality": row[4]}
                for row in rows
            ]
